        ...
        <tr>
            <td>
                <table class="table" style="width:100%">
                    <tr>
                        <th class="header-small" width="30%">Nom</th>
                        <th class="header-small" width="20%" colspan="2">Télécharger</th>
                    </tr>
...
&showresult=0">éditer</a>] [<a href="javascript:if(confirm('Voulez-vous vraiment supprimer la recherche enregistrée&nbsp;?'))location.href='query.php?queryid=<?php echo $this->_tpl_vars['queriesItem']['id']; ?>
&action=delete'">supprimer</a>]</span></nobr></td>
                        </tr>
                    <?php endforeach; else: ?>
                        <tr><td colspan="2"><br />Aucunes requêtes stockées trouvé</td></tr>
                    <?php endif; unset($_from); ?>
                    <?php if (count($this->_tpl_vars['queries']) > 0): ?>
                        <tr><td class="spacer">&nbsp;</td></tr>
                        <tr>
                            <td class="help" colspan=3">Avec le téléchargement, vous acceptez le terme <a href="articles.php?page=impressum#tos">d'utilisation</a> d'Opencaching.de.</td>
                        </tr>
                    <?php endif; ?>
                </table>
            </td>
...
/images/misc/32x32-search.png" style="margin-right: 10px;" width="32" height="32" alt="" />
        ...
        <table class="table">
            <tr>
                <td class="header-small" colspan="2">options de stockées comme nouvelle recherche</td>
            </tr>
            <tr>
                <td>Nom de la recherche&nbsp;:</td>
                <td>
...
" />
            ...
            <?php if ($this->_tpl_vars['errorEmptyName'] == true): ?>
                <tr><td colspan="2"><span class="errormsg">Vous devez entrer un nom pour cette recherche.</span></td></tr>
            <?php elseif ($this->_tpl_vars['errorNameExists'] == true): ?>
                <tr><td colspan="2" class="errormsg">Il existe déjà une recherche avec ce nom.</td></tr>
            <?php endif; ?>
            <tr>
                <td>&nbsp;</td>
                <td>
                    <input type="submit" name="savenew" value="stockées" class="formbutton" onclick="submitbutton('savenew')" />
                </td>
            </tr>
        </table>
    </form>
...
" />
        ...
        <table class="table">
            <tr>
                <td class="header-small" colspan="2">Ecrase les anciens options de recherche</td>
            </tr>
            <tr>
                <td>Nom de la recherche&nbsp;:</td>
                <td>
...
?>
                            <?php if (($this->_foreach['queries']['iteration'] <= 1)): ?>
                                <option value="0" selected="selected">-- Sélectionnez la recherche d'écraser --</option>
                            <?php endif; ?>
                            <option value="<?php echo $this->_tpl_vars['queriesItem']['id']; ?>
"><?php echo ((is_array($_tmp=$this->_tpl_vars['queriesItem']['name'])) ? $this->_run_mod_handler('escape', true, $_tmp) : smarty_modifier_escape($_tmp)); ?>
</option>
                        <?php endforeach; else: ?>
                            <option value="0">-- Aucune recherche stockées trouvé --</option>
                        <?php endif; unset($_from); ?>
                    </select>
                </td>
            </tr>
            <?php if ($this->_tpl_vars['errorMustSelectQuery'] == true): ?>
                <tr><td colspan="2" class="errormsg">Vous devez sélectionner une recherche pour écraser.</td></tr>
            <?php endif; ?>
            <tr>
                <td>&nbsp;</td>
                <td>
                    <input type="submit" name="overwrite" value="stockées" class="formbutton" onclick="submitbutton('overwrite')" />
                </td>
            </tr>
        </table>
    </form>
