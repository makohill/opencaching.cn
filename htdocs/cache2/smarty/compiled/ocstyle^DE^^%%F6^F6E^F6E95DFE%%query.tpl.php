...
&showresult=0">bearbeiten</a>] [<a href="javascript:if(confirm('Soll die gespeicherte Suche wirklich gel&ouml;scht werden?'))location.href='query.php?queryid=<?php echo $this->_tpl_vars['queriesItem']['id']; ?>
&action=delete'">löschen</a>]</span></nobr></td>
                        </tr>
                    <?php endforeach; else: ?>
                        <tr><td colspan="2"><br />Es sind keine gespeicherten Suchen vorhanden.</td></tr>
                    <?php endif; unset($_from); ?>
                    <?php if (count($this->_tpl_vars['queries']) > 0): ?>
                        <tr><td class="spacer">&nbsp;</td></tr>
                        <tr>
                            <td class="help" colspan=3">Mit dem Download werden die <a href="articles.php?page=impressum#tos">Nutzungsbedingungen</a> von Opencaching.de akzeptiert.</td>
                        </tr>
                    <?php endif; ?>
                </table>
            </td>
...
/images/misc/32x32-search.png" style="margin-right: 10px;" width="32" height="32" alt="" />
        ...
        <table class="table">
            <tr>
                <td class="header-small" colspan="2">Suchoptionen als neue Suche abspeichern</td>
            </tr>
            <tr>
                <td>Name der Suche:</td>
                <td>
...
" />
            ...
            <?php if ($this->_tpl_vars['errorEmptyName'] == true): ?>
                <tr><td colspan="2"><span class="errormsg">Es muss ein Name der Suche angegeben werden.</span></td></tr>
            <?php elseif ($this->_tpl_vars['errorNameExists'] == true): ?>
                <tr><td colspan="2" class="errormsg">Es existiert bereits eine Suche unter diesem Namen.</td></tr>
            <?php endif; ?>
            <tr>
                <td>&nbsp;</td>
                <td>
                    <input type="submit" name="savenew" value="Speichern" class="formbutton" onclick="submitbutton('savenew')" />
                </td>
            </tr>
        </table>
    </form>
...
" />
        ...
        <table class="table">
            <tr>
                <td class="header-small" colspan="2">Alte Suchoptionen überschreiben</td>
            </tr>
            <tr>
                <td>Name der Suche:</td>
                <td>
...
?>
                            <?php if (($this->_foreach['queries']['iteration'] <= 1)): ?>
                                <option value="0" selected="selected">-- Bitte zu überschreibende Suche auswählen --</option>
                            <?php endif; ?>
                            <option value="<?php echo $this->_tpl_vars['queriesItem']['id']; ?>
"><?php echo ((is_array($_tmp=$this->_tpl_vars['queriesItem']['name'])) ? $this->_run_mod_handler('escape', true, $_tmp) : smarty_modifier_escape($_tmp)); ?>
</option>
                        <?php endforeach; else: ?>
                            <option value="0">-- noch keine Suchoptionen vorhanden --</option>
                        <?php endif; unset($_from); ?>
                    </select>
                </td>
            </tr>
            <?php if ($this->_tpl_vars['errorMustSelectQuery'] == true): ?>
                <tr><td colspan="2" class="errormsg">Es muss eine zu überschreibende Suche gewählt werden.</td></tr>
            <?php endif; ?>
            <tr>
                <td>&nbsp;</td>
                <td>
                    <input type="submit" name="overwrite" value="Speichern" class="formbutton" onclick="submitbutton('overwrite')" />
                </td>
            </tr>
        </table>
    </form>
