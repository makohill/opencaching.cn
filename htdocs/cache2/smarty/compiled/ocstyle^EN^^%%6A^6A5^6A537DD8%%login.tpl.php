        ...
        <p style="line-height: 1.6em;" <?php if ($this->_tpl_vars['error'] != LOGIN_OK && $this->_tpl_vars['error'] != LOGIN_LOGOUT_OK): ?>class="errormsg"<?php endif; ?>>
            <?php if ($this->_tpl_vars['error'] != LOGIN_OK): ?>
                <?php if ($this->_tpl_vars['error'] == LOGIN_BADUSERPW): ?>
                    Login failed<br />
                    The entered username or password did not match.<br />
                <?php elseif ($this->_tpl_vars['error'] == LOGIN_TOOMUCHLOGINS): ?>
                    Login failed<br />
                    Your tried to login more than <?php echo $this->_tpl_vars['opt']['page']['max_logins_per_hour']; ?>
 times within the last hour. The next login will not be allowed before one hour since the first try has passed.<br />
                <?php elseif ($this->_tpl_vars['error'] == LOGIN_USERNOTACTIVE): ?>
                    Your account is not acivated.<br />
                    &gt;<a href="activation.php">Here</a>&lt; you can activate your account.<br />
                ...
                <?php else: ?>
                    登陆失败。<br />
                    If this problem persists over a longer time, please contact us via <a href="mailto:<?php echo $this->_tpl_vars['opt']['mail']['contact']; ?>
