...
<?php echo '
    function checkForm()
    {
        ...
        {
            alert(\''; ?>
Please name the picture!<?php echo '\');
            resetbutton(\'ok\');
            return false;
        }
        if (typeof FileReader !== "undefined") {
            ...
            if (size>maxsize){
                alert(\''; ?>
The file was too big. The maximum file size is <?php echo ((is_array($_tmp=$this->_tpl_vars['siteSettings']['logic']['pictures']['maxsize']/1024)) ? $this->_run_mod_handler('string_format', true, $_tmp, "%d") : smarty_modifier_string_format($_tmp, "%d")); ?>
 KB.<?php echo '\');
                return false;
            }
        }
...
/images/description/22x22-image.png" style="margin-right: 10px;" width="22" height="22" alt="Edit picture" title="Edit picture" />
        <?php ob_start(); ?>
...
"><?php echo ((is_array($_tmp=$this->_tpl_vars['cachename'])) ? $this->_run_mod_handler('escape', true, $_tmp) : smarty_modifier_escape($_tmp)); ?>
        ...
        <?php if ($this->_tpl_vars['action'] == 'add'): ?>
            <?php if ($this->_tpl_vars['objecttype'] == OBJECT_CACHE): ?>
                Add picture for geocache <?php echo $this->_smarty_vars['capture']['name']; ?>

            <?php else: ?>
                Add picture for log entry <?php echo $this->_smarty_vars['capture']['name']; ?>

            <?php endif; ?>
        <?php else: ?>
            <?php if ($this->_tpl_vars['objecttype'] == OBJECT_CACHE): ?>
                Edit picture for geocache <?php echo $this->_smarty_vars['capture']['name']; ?>

            <?php else: ?>
                Edit picture for log entry <?php echo $this->_smarty_vars['capture']['name']; ?>
...
" size="43" />
                <?php if ($this->_tpl_vars['errortitle'] == true): ?>
                    <span class="errormsg">Name the picture!</span>
                <?php endif; ?>
            </td>
        </tr>
        <?php if ($this->_tpl_vars['action'] == 'add'): ?>
...
" />
   ...
   <?php if ($this->_tpl_vars['errorfile'] == ERROR_UPLOAD_ERR_NO_FILE): ?>
                <tr><td>&nbsp;</td><td colspan="2"><span class="errormsg">No picture file given.</span></td></tr>
            <?php elseif ($this->_tpl_vars['errorfile'] == ERROR_UPLOAD_ERR_SIZE): ?>
                <tr><td>&nbsp;</td><td colspan="2"><span class="errormsg">The file was too big. The maximum file size is <?php echo ((is_array($_tmp=$this->_tpl_vars['siteSettings']['logic']['pictures']['maxsize']/1024)) ? $this->_run_mod_handler('string_format', true, $_tmp, "%d") : smarty_modifier_string_format($_tmp, "%d")); ?>
 KB.</span></td></tr>
            <?php elseif ($this->_tpl_vars['errorfile'] == ERROR_UPLOAD_UNKNOWN): ?>
                <tr><td>&nbsp;</td><td colspan="2"><span class="errormsg">The file was not uploaded correctly.</span></td></tr>
...
. We recommend JPEG for photos.<br />
 ...
 KB. We recommend 640x480 pixels picture size.<br />
    ...
    <input type="submit" name="ok" value="<?php if ($this->_tpl_vars['action'] == 'add'): ?>Upload<?php else: ?>Submit<?php endif; ?>" class="formbutton" onclick="submitbutton('ok')" />
   </td>
  </tr>
    </table>
</form>